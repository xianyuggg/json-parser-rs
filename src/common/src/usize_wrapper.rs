...
impl USIZEWrapper {
    ...
    pub fn go_ahead(&mut self, bytes: &[u8]) -> &mut USIZEWrapper {
    ...
    pub fn go_ahead_by_times(&mut self, bytes: &[u8], times: isize) -> &mut USIZEWrapper {
        for _ in 0..times {
            if self.0 == bytes.len() - 1 {
                println!("Go ahead reach bound!");
            } else {
    ...
    pub fn trim_whitespace(&mut self, bytes: &[u8]) -> &mut USIZEWrapper {
    ...
    pub fn is_end(&self, bytes: &[u8]) -> bool {
